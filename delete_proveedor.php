...
<body>
    ...
    <main class="main_register_comercio">
        ...
        <form class="regiter_comercio_form" method="post" action="php/deleteProveedor.php">
            <h1 class="display-6 mb-3">DAR DE BAJA PROVEEDOR</h1>
            <div class="row mb-3">
                ...
                <button type="submit" class="btn btn-danger loginContainer_formularioContainer_submit" onclick="return confirm('¿Estas seguro que desea eliminar el proveedor?');">Eliminar</button>
            </div>
    ...
    <!-- JS -->
    <?php
    if (isset($_SESSION['estado_delete']) && $_SESSION['estado_delete'] == "DELETE_PROVEEDOR_OK") {
        $_SESSION['estado_delete'] = null;
        echo "<script>toastr.success('Proveedor eliminado correctamente')</script>";
    }
    if (isset($_SESSION['estado_delete']) && $_SESSION['estado_delete'] == "DELETE_PROVEEDOR_ERROR") {
        $_SESSION['estado_delete'] = null;
        echo "<script>toastr.error('No se pudo eliminar el proveedor','ERROR')</script>";
    }
    ?>
</body>
